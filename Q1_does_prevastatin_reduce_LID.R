...
# Mitch just updated the data on 11/6/2023
#
# Cowen 2024
#
#
...
USE_NON_PARAMETRIC <- TRUE # Need to do this as the field likes non-parametric for AIMS
...
#title_str = 'Axial'
#fname = 'KETStat_Axial Summary_mjb061123.xlsx' # new This is the most interesting one according to Torsten

#title_str = 'Choreiform'
#fname = 'KETStat_Choreiform Summary_mjb061123.xlsx' # new
...
a0 <- ggplot(data = D, aes(condition, AIMS, fill = condition, line = 'black')) + 
  ...
  geom_jitter(width = .18, size = 2, alpha=0.7,  color = 'gray')+
  stat_summary(fun.data = mean_se, geom = "errorbar",  width=.3, linewidth = 1) + 
...
my_comparisons <- list( c('V', 'P'), c('P', 'L'), c("V", "L") )
c14 <- ggplot(data = D_long %>% filter(Day == day_txt, condition %in% c('V', 'P', 'L')) , aes(condition, AIMS, fill = condition, line = 'black')) + 
  stat_summary(fun = mean, geom = "bar") + 
  #geom_jitter(position = position_jitterdodge(jitter.width = .18), size = 3,  color = 'gray')+
  geom_jitter(width = .18, size = 2, alpha=0.7,  color = 'gray')+
  stat_summary(fun.data = mean_se, geom = "errorbar",  width=.3, linewidth = 1) + 
    #stat_compare_means(method = "t.test",comparisons = my_comparisons, label = "p.signif",label.y = c(33, 35, 37)) +
...
#                    ncol = 3, nrow = 3, size = 14)


a <- plot_grid(
  a0, b0, c0,a7, b7, c7, a14, b14, c14,
  labels = c("A","B","C", "D", "E", "F","G", "H", "I" ),
  label_size = 18,
  align="hv"
)
...
#ggarrange(a0, b0, c0,a7, b7, c7, a14, b14,c14,
#          labels = c("A","B","C", "D", "E", "F","G", "H", "I" ),
#          ncol = 3, nrow = 3, size = 14)

ggsave('C:\\Temp\\Q1prevastatin.pdf', width=6, height=5.27, device=cairo_pdf)
#install.packages('svglite')
library('svglite')
ggsave('C:\\Temp\\Q1prevastatin.svg', width=6, height=5.27)
...
#kruskal.test(AIMS ~ condition, data = D)
#dunn.test(D$AIMS, D$condition, method="holm")

D <-D_long %>% filter(Day == dy & condition %in% c('V', 'K', 'KP'))
dunnTest(AIMS ~ condition, data = D, method="holm")

D <-D_long %>% filter(Day == dy & condition %in% c('V', 'K', 'KL')) 
dunnTest(AIMS ~ condition, data = D, method="holm")

D <- D_long %>% filter(Day == dy & condition %in% c('V', 'P', 'L'))
dunnTest(AIMS ~ condition, data = D, method="holm")
dunnTest(AIMS ~ condition, data = D)
...
################## Special for Torsten
D <-D_long %>% filter(Day == 'Day 0' & condition %in% c('V', 'P', 'KP')) 
dunnTest(AIMS ~ condition, data = D, method="holm")
